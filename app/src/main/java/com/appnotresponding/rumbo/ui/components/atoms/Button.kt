...
fun RumboButton(
    ...
    val textStyle =
        if (size == RumboButtonSize.Large) RumboTextStyle.Body else RumboTextStyle.Button
    ...
    when (style) {
        ...
        RumboButtonStyle.Primary -> {
            Button(
                ...
                colors = ButtonDefaults.buttonColors(
                    containerColor = if (loading) AppTheme.colorScheme.primary.copy(alpha = 0.6f) else AppTheme.colorScheme.primary,
                    contentColor = AppTheme.colorScheme.onPrimary,
                    disabledContainerColor = AppTheme.colorScheme.onSurface.copy(alpha = 0.3f),
                    disabledContentColor = AppTheme.colorScheme.onSurface.copy(alpha = 0.6f)
                )
            ) {
                ButtonContent(
                    text,
        ...
        RumboButtonStyle.Secondary -> {
            OutlinedButton(
                ...
                border = BorderStroke(
                    1.dp,
                    if (enabled && !loading) AppTheme.colorScheme.outline
                    else if (loading) AppTheme.colorScheme.outline.copy(alpha = 0.5f)
                    else AppTheme.colorScheme.onSurface.copy(alpha = 0.3f)
                ),
                colors = ButtonDefaults.outlinedButtonColors(
                    contentColor = if (loading) AppTheme.colorScheme.primary.copy(alpha = 0.6f) else AppTheme.colorScheme.primary,
                    disabledContentColor = AppTheme.colorScheme.onSurface.copy(alpha = 0.6f)
                )
            ) {
                ButtonContent(
                    text,
                    textStyle,
                    if (loading) AppTheme.colorScheme.primary.copy(alpha = 0.6f) else AppTheme.colorScheme.primary,
                    loading,
